        ...
        <div>
          ...
          <table className="table">
            ...
            <thead><tr><th> Id </th><th> Nom </th><th> Description </th><th> Adresse </th><th> Logo </th><th> Réseau Social Url 1 </th><th> Réseau Social Url 2 </th>
              <th> Réseau Social Url 3 </th><th> Téléphone </th><th> Site Internet </th><th> Mail </th><th> Departements Id </th><th> Actions </th><th>Ajout/Supp</th></tr></thead>
            <tbody>
              {this.state.assoProfil.map((el, index) =>
                <tr><td>{el.id}</td><td>{el.name}</td><td>{el.description}</td><td>{el.address}</td><td>{el.logo}</td>
                  ...
                  <td>{el.phone_number}</td><td>{el.web_site}</td><td>{el.mail}</td><td>{el.departements_id}</td><td>{el.action}</td><td><Link to={'/modifyAssoprofil/' + el.id}>
