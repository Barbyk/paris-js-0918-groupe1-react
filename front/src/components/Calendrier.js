...
class Calendrier extends PureComponent {
  ...
  getLocations = () => {
    ...
    axios.get('/api/public/locations/')
      .then(response => {
        this.setState({
  ...
  getEvents = (locationid = 0) => {
    ...
    axios.get('/api/public/events/location/' + locationid)
      .then(response => {
  ...
  getMultipleEvents = async (locationid = 0) => {
    ...
    await axios.post('/api/public/events/multiplelocations/', {id:locationid})
      .then(response => {
        if (response.data){
        let appointments = response.data;
        if (appointments)
  ...
  editEvent = (element) => {
    ...
    axios.put('/api/public/events/' + this.state.currentEvent.id, {
